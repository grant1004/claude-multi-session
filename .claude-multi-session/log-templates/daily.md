...
- **<file or rule>** — <reason, e.g. "Domain layer must not reference WPF, design decision 2026-05-14">
...
- Before adding a new converter: check `App.xaml` (or equivalent) for already-registered ones (don't duplicate)
...
### `<hot file path>`
- **State now:** <key state, e.g. "6 DependencyProperties defined: A, B, C, D, E, F">
- **Pattern asymmetry:** <e.g. "GroupColor mode doesn't read PulsePhase; Dashboard mode doesn't read Groups outline">
- **Performance note:** <e.g. "PulsePhase animates forever via BeginAnimation; harmless under 1k points, replace with DrawingVisual + VisualHost above 10k">
- **Upgrade path (in code comment):** <if any>

### `<new sub-product introduced this session>` — sub-product
- **Registered at:** <e.g. `App.xaml`: `<local:FooConverter x:Key="FooConverter" />`>
...
- **<deferred quality concern>**: 
  - <item 1, e.g. "M9.2 animation always runs → enable/disable when point count grows past 10k">
  - <item 2>
```
