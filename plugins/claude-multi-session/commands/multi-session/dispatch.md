...
🤖 Auto-pass criteria (for 🤖-marked milestones only — omit this section entirely for ✍️ milestones):
...
- **Rule 2 (build)**: adapt the `<build command>` to the project. If `package.json` exists, use `npm run build` or `npm test`. If no build step (pure markdown projects), say "No build step — but verify <appropriate check>". Read from PROGRESS.md audit summary or `CLAUDE.md` for the project's build command.
- **Rule 6 (acceptance criteria tests)**: only applies when acceptance criteria include executable tests. If not, the rule still appears (template requires all seven) but is a no-op for the Worker.
- **Auto-pass criteria section**: include for 🤖-marked milestones only. Omit the section entirely for ✍️ milestones.
